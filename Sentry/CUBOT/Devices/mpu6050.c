...
static uint8_t MPU6050_ReadData(uint8_t reg_add, uint8_t *Read, uint8_t num,I2C_HandleTypeDef *hi2c,Device_MODE mode)
{
	...
	HAL_I2C_Mem_Read_DMA(&hi2c2, MPU6050_ADDRESS,reg_add,I2C_MEMADD_SIZE_8BIT,Read, num);
}
...
void MPU6050_Calibrate_Offset(IMU_InitData_t *mpu6050_data)
{
    ...
    do {
        ...
        mpu6050_cali_count++;
             ...
             fabsf(mpu6050_data->gyro_offset[2]) > 0.07f);
...
void MPU6050_Read(IMU_InitData_t *mpu6050_data,Device_MODE mode)
{
    ...
    if (abs(mpu6050_data->gyro[2]/MPU6050_GYRO_SEN)<20) mpu6050_data->gyro[2]=0;
}
...
void IMUupdate_1(IMU_InitData_t *mpu6050_data)
{
   ...
   float  halfT,norm,vx,vy,vz,ex,ey,ez,tempq0,tempq1,tempq2,tempq3,now,dt;
    ...
    tempq0 = q0_1 + (-q1_1*mpu6050_data->gyro[0] - q2_1*mpu6050_data->gyro[1] - q3_1*mpu6050_data->gyro[2])*dt*0.5;
    tempq1 = q1_1 + (q0_1*mpu6050_data->gyro[0] + q2_1*mpu6050_data->gyro[2] - q3_1*mpu6050_data->gyro[1])*dt*0.5;
    tempq2 = q2_1 + (q0_1*mpu6050_data->gyro[1] - q1_1*mpu6050_data->gyro[2] + q3_1*mpu6050_data->gyro[0])*dt*0.5;
    tempq3 = q3_1 + (q0_1*mpu6050_data->gyro[2] + q1_1*mpu6050_data->gyro[1] - q2_1*mpu6050_data->gyro[0])*dt*0.5;  
    ...
    q3_1= tempq3 * norm;
		...
		mpu6050->Yaw=-atan2(2 * q1_1 * q2_1+ 2 * q0_1* q3_1, -2 * q2_1*q2_1- 2 * q3_1 * q3_1 + 1)*57.3; // yaw        -pi----pi
    mpu6050->Roll= -asin(-2 * q1_1 * q3_1+ 2 * q0_1 * q2_1)*57.3; // pitch    -pi/2    --- pi/2 
//		//>pitch轴角度
    mpu6050->Pitch= -atan2(2 * q2_1* q3_1 + 2 * q0_1* q1_1, -2 * q1_1 * q1_1 - 2 * q2_1* q2_1+ 1)* 57.3; 
